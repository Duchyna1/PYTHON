...
line1 = [str(x) for x in input('line1: ').split(',')]
line2 = [str(x) for x in input('line2: ').split(',')]
...
def Up(X, Y, kolko, line):
    print('U', line, end='')
    for n in range(Y-kolko-1, Y-1):
        if line == 1:
            pos_line1.append(f'{X},{n}')
        if line == 2:
            if f'{X},{n}' in pos_line1:
                distance.append(abs(X)+abs(n))
    return X, Y-kolko


def Down(X, Y, kolko, line):
    print('D', line, end='')
    for n in range(Y+1, Y+kolko+1):
        if line == 1:
            pos_line1.append(f'{X},{n}')
        if line == 2:
            if f'{X},{n}' in pos_line1:
...
def Left(X, Y, kolko, line):
    print('L', line, end='')
    for n in range(X-kolko-1, X-1):
        if line == 1:
            pos_line1.append(f'{n},{Y}')
        if line == 2:
            if f'{n},{Y}' in pos_line1:
                distance.append(abs(n)+abs(Y))
    return X-kolko, Y


def Right(X, Y, kolko, line):
    print('R', line, end='')
    for n in range(X+1, X+kolko+1):
        if line == 1:
            pos_line1.append(f'{n},{Y}')
        if line == 2:
            if f'{n},{Y}' in pos_line1:
...
for smer in line1:
    if 'U' in smer:
        kdeX, kdeY = Up(kdeX, kdeY, int(smer[1:]), 1)
    if 'D' in smer:
        kdeX, kdeY = Down(kdeX, kdeY, int(smer[1:]), 1)
    if 'R' in smer:
        kdeX, kdeY = Right(kdeX, kdeY, int(smer[1:]), 1)
    if 'L' in smer:
        kdeX, kdeY = Left(kdeX, kdeY, int(smer[1:]), 1)
...
for smer in line2:
    if 'U' in smer:
        kdeX, kdeY = Up(kdeX, kdeY, int(smer[1:]), 2)
    if 'D' in smer:
        kdeX, kdeY = Down(kdeX, kdeY, int(smer[1:]), 2)
    if 'R' in smer:
        kdeX, kdeY = Right(kdeX, kdeY, int(smer[1:]), 2)
    if 'L' in smer:
        kdeX, kdeY = Left(kdeX, kdeY, int(smer[1:]), 2)
