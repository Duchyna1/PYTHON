...
def up(x, y, step, line):
    global steps
    for n in range(1, step+1):
        steps += 1
        if line == 1:
            if f'{x},{y-n}' not in coords:
                coords.update({f'{x},{y-n}': steps})
        if line == 2:
            if f'{x},{y-n}' in coords:
                print(f'{x},{y-n}', coords.get(f'{x+n},{y}'))
                distances.append(steps+coords.get(f'{x},{y-n}'))
    return X, Y-step


def down(x, y, step, line):
    global steps
    for n in range(1, step+1):
        steps += 1
        if line == 1:
...
def left(x, y, step, line):
    global steps
    for n in range(1, step+1):
        steps += 1
        if line == 1:
            if f'{x-n},{y}' not in coords:
                coords.update({f'{x-n},{y}': steps})
        if line == 2:
            if f'{x-n},{y}' in coords:
                print(f'{x-n},{y}', coords.get(f'{x+n},{y}'))
                distances.append(steps+coords.get(f'{x-n},{y}'))
    return x-step, y


def right(x, y, step, line):
    global steps
    for n in range(1, step+1):
        steps += 1
        if line == 1:
...
for move in line1:
    if 'U' in move:
        X, Y = up(X, Y, int(move[1:]), 1)
    if 'D' in move:
        X, Y = down(X, Y, int(move[1:]), 1)
    if 'L' in move:
        X, Y = left(X, Y, int(move[1:]), 1)
    if 'R' in move:
        X, Y = right(X, Y, int(move[1:]), 1)

X, Y, steps = 0, 0, 0

for move in line2:
    if 'U' in move:
        X, Y = up(X, Y, int(move[1:]), 2)
    if 'D' in move:
        X, Y = down(X, Y, int(move[1:]), 2)
    if 'L' in move:
        X, Y = left(X, Y, int(move[1:]), 2)
    if 'R' in move:
        X, Y = right(X, Y, int(move[1:]), 2)
